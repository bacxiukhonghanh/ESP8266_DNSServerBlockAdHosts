...
void MyDNSServer::respondToRequest(uint8_t *buffer, size_t length)
{
  ...
  */
  ...
  /*
   * The domain was not found in the blacklist. 
   * I will use the external DNS server to look up its IP address and return the result.
   * In case the lookup fails, I will reply with an error so that the client
   * can refer to its secondary DNS server as a backup.
  */
  Serial.println(queryProcessedString + " was not found. Passing to the external DNS server.");
  IPAddress resolvedExternalIP;
  if (!WiFi.hostByName(queryProcessedString.c_str(), resolvedExternalIP)) {
    Serial.println("Error: " + queryProcessedString + ": DNS lookup failed.");
    replyWithError(dnsHeader, _errorReplyCode, query, queryLength);
    return BlinkLEDOnRequests();
  }
  else {
    Serial.print(queryProcessedString + ": ");
    Serial.println(resolvedExternalIP);
    unsigned char addrToReply[4] = { resolvedExternalIP[0], resolvedExternalIP[1], resolvedExternalIP[2], resolvedExternalIP[3] };
    replyWithIP(dnsHeader, query, queryLength, addrToReply);
    return BlinkLEDOnRequests();
  }
}
